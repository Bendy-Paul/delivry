...
$truckstate=$check1->fetch_assoc();
	...
	$package=$conn->query("SELECT * FROM d_orders WHERE delivery_truck_number='$id' AND (item_status!='8' AND item_status!='9')");
	$deliverystate = $package->fetch_assoc();


if (($truckstate['delivery_state'] != $_POST['state']) || ($truckstate['driver_office_branch_id'] != $_POST['current_state'])) {
	if ($package->num_rows>0) {
	$error=true;
	$stateE="Cannot Change State Information while truck is still delivering packages <br>";
}else {
	$dcurrent_state=$_POST['current_state'];
	$dstate=$_POST['state'];
}
}else {
	$dstate=$_POST['state'];
...
$sql=$conn->query("UPDATE delivery_trucks SET dcar_id='$plate_number',carperformance='$peform', delivery_state='$dstate', driver_office_branch_id='$dcurrent_state',driver_location_status='1' WHERE dcar_id='$id';");
...
<a class="btn btn-block btn-sm btn-default btn-flat border-primary btn-primary" href="newpackage1.php"><i class="fa fa-plus"></i> Add New</a>
    ...
    <script src="search.js"></script>
				...
				<tbody>
					<?php
					$i = 1;
					$qry = $conn->query("SELECT * FROM delivery_trucks  WHERE driver_location_status='3' OR driver_location_status='2' ORDER BY carbrand ASC; ");
...
?>
                    ...
                    <td><b><?php echo $row['carbrand'] ?></b></td>
                        ...
                        <div class="btn-group"> <button value="<?php echo $row['dcar_id']?>" name="name" class="btn btn-info btn-flat view_parcel" data-toggle="modal" data-target="#<?php echo $row['dcar_id']?>"
						<?php
if ($row['driver_location_status']=="2") {
							?>
disabled
<?php
}
...
<div class="modal-dialog">
  ...
  <div class="form-group">
    ...
    <select name="current_state" id="" class="form-control">

		<option class="btn btn-secondary" value="<?php echo $row['driver_office_branch_id'] ?>"><?php echo $row['driver_office_branch_id'] ?></option>
